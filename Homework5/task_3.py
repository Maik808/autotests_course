...
def everything_for_your_cat(cats_data):
    """
    Функция упорядочивает данные, у каждого владельца отображаются все его питомцы отдельной строкой
    :param cats_data: Входные данные: списки кортежей с данными о кошках и их владельцах
    :return:
    """
    # Здесь нужно написать код
    dictionary ={}
    for cat in cats_data:
        full_name = ("{} {}".format(*cat[2:]))
        dictionary[full_name] = {}
    for cat in cats_data:
        full_name = ("{} {}".format(*cat[2:]))
        dictionary[full_name][cat[0]] = cat[1]

    our_str = ''
    for buyer, cats in dictionary.items():
        cats_info = '; '.join(['{}, {}'.format(cat, age) for cat, age in cats.items()])
        our_str += ('{}: {}\n'.format(buyer, cats_info))
    return our_str
