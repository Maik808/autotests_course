...
class PersonInfo:
    ...
    def short_name(self):
        ...
        """
        variable = self.full_name.split(" ")
        return f'{variable[1]} {variable[0][0]}.'

    def path_deps(self):
        """
        Метод возвращает строку "Головное подразделение --> Подразделение --> Конечное подразделение"
        :return: Строка с подразделениями
        """
    ...
    def new_salary(self):
        ...
        :return: Сумма зп после индексации
        """
        lline = list(''.join(self.departments))
        string_set = set(lline)
        letters_dict = {}
        for x in string_set:
            letters_dict[x] = lline.count(x)
        sorted_letters = sorted(letters_dict.items(), key=lambda y: y[1], reverse=True)
        top_letters = sorted_letters[:3]
        top_letters_count = sum([letter[1] for letter in top_letters])
        return 1337 * self.age * top_letters_count
